class  digitRegex{
    ...
    minThan(digit){
        try{
            ...
            let listMax = this.minRegex(String(digit));
    ...
    maxThan(digit){
        try{
            ...
            var  patterns = ['(?:(?=(^[1-9][0-9]*$))|(?=(^0$)))'];  //just digit
            //todo grath equal than maxDigit
            let listMax = this.maxRegex(String(digit));
            patterns.push(`(?:${listMax.join('|')})`);
            //todo make regex
            return new RegExp(`(?:${patterns.join('')})`, 'g');
        }
    ...
    betweens(minDig,maxDig){
        try{
            ...
            var listMax = this.maxRegex(String(minDig));
            patterns.push(`(?:${listMax.join('|')})`);

            //todo lower equal than maxDigit
            var listMax = this.minRegex(String(maxDig));
            patterns.push(`(?:${listMax.join('|')})`);
